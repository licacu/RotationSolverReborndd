...
[Rotation("MCH Reborn Potion+", CombatType.PvE, GameVersion = "7.4")]
[SourceCode(Path = "main/RebornRotations/Ranged/MCH_Reborn_Potion.cs")]

public sealed class MCH_Reborn_Potion : MachinistRotation
{
    ...
    // === NEW OPTION ===
    [RotationConfig(CombatType.PvE, Name = "Second Potion Window (Mid-fight)")]
    private PotionTime SecondPotion { get; set; } = PotionTime.SixthMin;

    public enum PotionTime : byte
    {
        [Description("None")] None,
        [Description("6th Minute (Opener + 6min)")] SixthMin,
        [Description("8th Minute (Opener + 8min)")] EighthMin
    }
    // ==================
    ...
    {
        // === NEW POTION LOGIC START ===
        // Checks if we should pop a potion mid-fight (6th or 8th minute)
        // Only uses it if we are in Burst mode and combat time is within the window
        if (IsBurst && InCombat && HasHostilesInRange && SecondPotion != PotionTime.None)
        {
            bool inWindow = false;
            // 6th Minute Window: 350s to 440s (allows for delay)
            if (SecondPotion == PotionTime.SixthMin && CombatTime >= 350f && CombatTime < 440f) inWindow = true;
            
            // 8th Minute Window: 470s to 560s
            if (SecondPotion == PotionTime.EighthMin && CombatTime >= 470f && CombatTime < 560f) inWindow = true;

            if (inWindow && !StatusHelper.PlayerHasStatus(true, StatusID.Medicated))
            {
                if (UseBurstMedicine(out act)) return true;
            }
        }
        // === NEW POTION LOGIC END ===
    ...
    public override void DisplayRotationStatus()
    {
        ...
        ImGui.Separator();
        ImGui.TextColored(ImGuiColors.DalamudYellow, "Potion Status:");
        ImGui.Text($"Setting: {SecondPotion}");
        bool inWindow = false;
        if (SecondPotion == PotionTime.SixthMin && CombatTime >= 350f && CombatTime < 440f) inWindow = true;
        if (SecondPotion == PotionTime.EighthMin && CombatTime >= 470f && CombatTime < 560f) inWindow = true;
        
        ImGui.Text($"In Potion Window: {inWindow}");
        ImGui.Text($"Has Medicated: {StatusHelper.PlayerHasStatus(true, StatusID.Medicated)}");
    }
    #endregion
